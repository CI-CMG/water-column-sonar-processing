...
@pytest.fixture
def raw_to_zarr_remote_test_path(test_path):
    return test_path["RAW_TO_ZARR_REMOTE_TEST_PATH"]
...
def test_raw_to_zarr(moto_server, raw_to_zarr_test_path):
    ...
    assert (
        "data/raw/Henry_B._Bigelow"
        in s3_manager.list_objects(bucket_name=input_bucket_name, prefix="")[0]
    )
...
@mock_aws
def test_raw_to_zarr_hb1006(moto_server, raw_to_zarr_remote_test_path):
    """
    includes test file with missing data in lat/lon so ping_time count does not align
    and ping_time_dropna is used.
    """
    table_name = "water-column-sonar-table"
    s3_manager = S3Manager(endpoint_url=moto_server)
    ...
    s3_manager.upload_file(
        filename=raw_to_zarr_remote_test_path.joinpath(
            "HBB-D20100720-T153257.raw"
        ),  # "./test_resources/D20070724-T042400.raw",
        bucket_name=input_bucket_name,
        key="data/raw/Henry_B._Bigelow/HB1006/EK60/HBB-D20100720-T153257.raw",
    )
    assert len(s3_manager.list_objects(bucket_name=input_bucket_name, prefix="")) == 1
    assert (
        "data/raw/Henry_B._Bigelow"
        in s3_manager.list_objects(bucket_name=input_bucket_name, prefix="")[0]
    )
    ...
    # file_name = "D20070711-T182032.raw"
    # file_name = "D20070720-T224031.raw" # 84 KB
    raw_file_name = "HBB-D20100720-T153257.raw"  # 1.5 MB use this for testing
    # bottom_file_name = f"{Path(raw_file_name).stem}.bot"
    ...
    # Ensure that all the files were uploaded properly
    # assert len(number_of_files) == 72
    assert len(number_of_files) == 283  # 86 files

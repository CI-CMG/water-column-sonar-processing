...
from src.model.aws.s3fs_manager import S3FSManager
...
def test_add_file(tmp_path):
    ...
    zarr_path = f'{temporary_directory}/example.zarr'
    ds = xr.Dataset(
        {
    ...
    # --- Upload to S3 --- #
    s3fs_operation = S3FSManager()
    # TODO: just copy from a to b
    # foo = s3_manager.upload_files_to_bucket(local_directory=zarr_path, object_prefix='ship/cruise/sensor/example.zarr', bucket_name=test_bucket_name)
    # s3_manager.upload_file(zarr_path + '/.zmetadata', test_bucket_name, 'ship/cruise/sensor/example.zarr/.zmetadata')

    # s3fs_operation.upload_data(
    #     bucket_name=test_bucket_name,
    #     file_path=zarr_path,
    #     file_name='ship/cruise/sensor/example.zarr'
    # )

    # s3_object = s3_manager.get(bucket_name=test_bucket_name, key="ship/cruise/sensor/example.zarr/.zmetadata")
    ...
    # assert s3_manager.folder_exists_and_not_empty(test_bucket_name, "/example.zarr")
    # assert s3fs_operation.exists(f"{test_bucket_name}/ship/cruise/sensor/example.zarr")
    #
    #
    ...
    s3fs_operation.upload_data(bucket_name=test_bucket_name, file_path=zarr_path, prefix="ship/cruise/sensor")

    found = s3_manager.list_objects(test_bucket_name, 'ship/cruise/sensor/example.zarr')
    print(found)
    s3_object = s3_manager.get(bucket_name=test_bucket_name, key="ship/cruise/sensor/example.zarr/.zgroup")
    body = s3_object.get()["Body"].read().decode("utf-8")
    print(body)

    s3_store = s3fs_operation.s3_map(s3_zarr_store_path=f"s3://{test_bucket_name}/ship/cruise/sensor/example.zarr")
